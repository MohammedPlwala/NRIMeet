...
<div class="form-group">
    ...
    <div class="form-control-wrap">
        ...
        <input type="text" @if($required !='' && $required=='true' ) required="{{$required}}" @endif data-parsley-errors-container=".parsley-container-{{ $name }}" id="{{$for}}" value="{{$value}}" placeholder="{{ $placeholder }}" name="{{ $name }}"  {{ $attributes }} {{ $attributes->merge(['class' => 'form-control ' . $class]) }} autocomplete="off" @if($readonly == 'true') readonly @endif/>
    </div>
